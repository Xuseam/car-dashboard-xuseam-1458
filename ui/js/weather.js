		...
		language: language
  		...
  		if (err) {
  			...
  			if (data.forecasts) {
                ...
                var yearNum = parseInt($("#year-input").val());
				var monthNum = parseInt($("#month-input").val())-1;
				var dayNum = parseInt($("#day-input").val());
                var day2 = (new Date).setFullYear(yearNum, monthNum, dayNum);
                var number_of_days = Math.floor((day2 - day1) / 86400000);

                $("#temperature_label").html(data.forecasts[number_of_days].max_temp+"/"+data.forecasts[number_of_days].max_temp);
                $("#weather_label").html(data.forecasts[number_of_days].narrative);
	...
	weatherAPI("/api/forecast/hourly", { 
		...
		language: language
  		...
  		if (err) {
  			...
  			if (data.observation) {
                $("#temperature_label").html(data.observation.temp);
                $("#weather_label").html(data.observation.wx_phrase);
            } else {
